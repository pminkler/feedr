...
test('add-recipe-modal-guest', async ({ page }) => {
  ...
  try {
    ...
    warnLog(`Error interacting with recipe URL input: ${error}`);
...
test('add-recipe-modal-authenticated', async ({ page, isMobile, browserName }) => {
  ...
  if (isMobile || browserName === 'webkit') {
    ...
    try {
      ...
      try {
        ...
        warnLog(`Mobile recipe verification failed: ${error}`);
      }
  ...
  for (let attempt = 0; attempt < 3 && !buttonClicked; attempt++) {
    try {
      ...
      verboseLog(`Attempt ${attempt + 1} to click Add Recipe button failed: ${error}`);
  ...
  try {
    ...
    const modalInputPromise = new Promise((resolve, reject) => {
      // Use IIFE pattern instead of async promise executor
      (async () => {
        try {
    ...
    warnLog(`Error adding recipe URL: ${error}`);
