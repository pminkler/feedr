...
export function useRecipe() {
  ...
  async function getSavedRecipes() {
    try {
      ...
      console.log(
        `Found ${savedRecipes.length} saved recipes using identity ID ${identityId}`,
      );
      ...
      if (savedRecipes.length > 0) {
        ...
        console.log(
          "Trying fallback query with username for saved recipes:",
          username,
        );
        ...
        if (fallbackRecipes.length > 0) {
          console.log(
            "Sample fallback saved recipe owners:",
            fallbackRecipes[0].owners,
          );
  ...
  async function getMyRecipes() {
    try {
      ...
      console.log(
        `Found ${myRecipes.length} recipes using identity ID ${identityId}`,
      );
      ...
      if (myRecipes.length > 0) {
        ...
        const fallbackRecipes = fallbackResponse.data || [];
        console.log(
          `Found ${fallbackRecipes.length} recipes using username fallback ${username}`,
        );

        if (fallbackRecipes.length > 0) {
          console.log(
            "Sample fallback recipe owners:",
            fallbackRecipes[0].owners,
          );
    ...
    try {
      ...
      if (error.response) {
        try {
          console.log(
            "Error details: ",
            JSON.parse(await error.response.body.text()),
          );
        } catch (e) {
