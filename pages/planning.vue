...
onMounted(async () => {
  ...
  try {
    ...
    if (!mealPlanError.value && !hasTriedCreatingPlan.value && mealPlansState.value.length === 0) {
    ...
    <template #header>
      ...
      >
        <div class="w-full flex flex-wrap items-center gap-3">
            ...
            <template #item="{ item, selected }">
              <div class="flex items-center gap-2">
                ...
                <div
                  ...
                  :style="{
                    backgroundColor:
                      mealPlansState.find((p) => p.name === item)?.color || '#3B82F6',
                  }"
                ></div>
          ...
          <div class="ml-auto">
            <UButton color="primary" variant="soft" icon="i-heroicons-plus" size="sm">
              Add Plan
            </UButton>
          </div>
        </div>
          ...
          <div v-if="mealPlansState.length > 0" class="space-y-4">
            ...
            <div class="flex justify-between items-center mb-6">
              ...
              <div class="flex items-center gap-2">
                ...
                <UButton variant="ghost" color="primary" @click="goToCurrentWeek" size="sm">
                  Today
                </UButton>
            ...
            <div class="grid grid-cols-1 md:grid-cols-7 gap-4">
              <div v-for="day in visibleWeekDays" :key="day.date" class="space-y-2">
                <!-- Day header -->
                <div
                ...
                <!-- Single day card -->
                <UCard class="h-64 overflow-y-auto" :ui="{ body: 'p-1 sm:p-1' }">
                  ...
                  >
                    ...
                    <div v-else class="p-1 space-y-1">
                      ...
                      <div
                        v-for="assignment in getMealAssignmentsForDate(day.date)"
                        :key="assignment.id"
                      ...
                      >
                        ...
                        <div class="relative">
                          <UChip
                            class="w-full"
                            :color="
                              mealPlansState.find((p) => p.id === assignment.mealPlanId)?.color ||
                              'blue'
                            "
                            variant="subtle"
                            :ui="{
                          ...
                          >
                            <div class="flex items-center w-full gap-1">
                              ...
                              <UBadge
                                :color="
                                  mealPlansState.find((p) => p.id === assignment.mealPlanId)
                                    ?.color || 'blue'
                                "
                                variant="solid"
                                size="xs"
                              ...
                              <div class="flex-1 min-w-0">
                                <!-- Recipe title -->
                                <span class="text-[11px] font-medium block truncate leading-tight">
                                  {{ assignment.recipe?.title || 'Unnamed recipe' }}
                                </span>

                                <!-- Plan name in tiny text -->
                                <span class="text-[8px] opacity-70 block truncate leading-tight">
                                  {{
                                    mealPlansState.find((p) => p.id === assignment.mealPlanId)
                                      ?.name || 'Unknown plan'
                                  }}
